...
newtype LamportClockSim s a =
    LamportClockSim (RWS s () (Map Pid (ProcessState s)) a)
...
newtype ProcessSim s a = ProcessSim (ReaderT Pid (LamportClockSim s) a)
    deriving (Applicative, Functor, Monad)

instance Process (ProcessSim s) where
    getPid = ProcessSim ask

instance Clock (ProcessSim s) where
...
instance MonadState s (ProcessSim s) where
    get = ProcessSim $ do
        pid <- ask
        lift . LamportClockSim $ do
            initial <- ask
...
preIncrementTime :: Pid -> LamportClockSim s LocalTime
